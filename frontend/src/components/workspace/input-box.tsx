    ...
    <div ref={promptRootRef} className="relative">
      ...
      >
          ...
          >
            <div className="flex flex-wrap items-start gap-2">
              {selectedDataSources.map((source) => (
                <div
                  key={source.id}
                  className="flex max-w-[18rem] min-w-0 items-start gap-2 rounded-2xl border bg-white/90 px-3 py-2 shadow-sm"
                >
                  <button
                    type="button"
        ...
        <PromptInputBody
          ...
          style={{
            top: selectionAreaHeight > 0 ? `${selectionAreaHeight + 8}px` : 0,
          }}
        >
          <PromptInputTextarea
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-96">
                {MANUAL_COMPLIANCE_IDENTITIES.map((identity) => (
                  ...
                  >
                    <ShieldCheckIcon className="mt-0.5 size-4 self-start" />
                    <div className="flex min-w-0 flex-1 flex-col">
                      <span>{identity.label}</span>
                      <span className="text-muted-foreground text-[10px] leading-4">
                        {identity.sceneLabel}｜{identity.audience}
                      </span>
                      <span className="text-muted-foreground text-[10px] leading-4 whitespace-normal">
                        适用：{identity.whenToUse}
                      </span>
                    </div>
                ...
                <DropdownMenuLabel className="text-muted-foreground text-xs">
                  已绑定场景：{selectedComplianceScene.label}
                </DropdownMenuLabel>
                <div className="text-muted-foreground px-2 py-1 text-[11px] leading-4">
                  {selectedComplianceIdentity.description}
                </div>
                <DropdownMenuSeparator />
                <div className="text-muted-foreground px-2 py-1 text-[11px] leading-4">
                  身份与场景按指南固定绑定，后端会再次校验。当前仍是测试画像，不代表真实
                  IAM 授权。
                </div>
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        <DialogContent className="sm:max-w-4xl">
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative min-w-[16rem] flex-1">
                ...
                <input
                  ...
                  className="bg-background h-10 w-full rounded-xl border pr-3 pl-9 text-sm transition outline-none focus:border-neutral-400"
                />
              </div>
              <Button
                type="button"
            ...
            <div className="overflow-hidden rounded-2xl border">
              <div className="text-muted-foreground bg-muted/40 grid grid-cols-[56px_minmax(0,1.6fr)_120px_180px] border-b px-4 py-3 text-xs font-medium">
                <div />
              ...
              <div className="max-h-[24rem] overflow-y-auto">
                {dialogSources.length > 0 ? (
                  dialogSources.map((source) => {
                    const selected = draftSelectedDataSourceIds.includes(
                      source.id,
                    );
                    return (
                      <button
