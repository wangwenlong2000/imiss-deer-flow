...
| A1b | 上传描述 + 画面质量 + 事件 -> 健康检查与事件分析同时入链 | pass | {"capability": "camera_health_operations", "recommended_skill_chain": ["frame-sampling", "camera-health-check", "single-video-event-analysis"]} |
| A1c | 真实入库请求仍路由到 video_library_governance（回归） | pass | {"capability": "video_library_governance", "recommended_skill_chain": ["video-stream-ingestion", "batch-video-ingestion", "video-embedding-index"]} |
...
| A13b | 视频 bundle 完整性：SKILL.md、脚本引用与语法 | pass | {"skills": 21, "scripts_compiled": 21} |
