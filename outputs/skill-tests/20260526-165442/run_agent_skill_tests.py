...
PY""",
        "batch-video-ingestion": f"""mkdir -p {out} {ws}
python - <<'PY'
import json, pathlib
videos = {{"videos":[{{"video_id":"video-traffic-agent-test","camera_id":"{CAMERA_ID}","file_path":"{TRAFFIC_VIDEO}","source_type":"local_file","capture_seconds":2,"metadata":{{"filename":"Trafic.mp4","location":{{"city":"宁波市","district":"奉化区"}}}}}}]}}
p = pathlib.Path('{ws}/videos.json')
p.parent.mkdir(parents=True, exist_ok=True)
...
python - <<'PY'
import json, pathlib
event = {{"event_id":"EVT_PACKAGE_001","video_id":"video-traffic-agent-test","camera_id":"{CAMERA_ID}","event_type":"manual_evidence","event_time":"2026-05-20T10:00:01+08:00","event_elapsed_seconds":1,"image_uri":"{ws}/source.jpg","raw_segment_uri":"{TRAFFIC_VIDEO}","evidence_frame_ids":["frame_001"]}}
p = pathlib.Path('{ws}/event.json')
p.parent.mkdir(parents=True, exist_ok=True)
json.dump(event, p.open('w'), ensure_ascii=False, indent=2)
PY
