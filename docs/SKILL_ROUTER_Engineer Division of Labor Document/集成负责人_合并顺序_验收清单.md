...
| `.env.example` | 环境变量 | 保留 `NETWORK_TRAFFIC_ES_INDEX` 和 `SKILL_ROUTER_ES_INDEX` |
...
2. NETWORK_TRAFFIC_ES_INDEX 继续保留给 RAG
...
12. 你好 trigger=false
13. 不写入、不污染 NETWORK_TRAFFIC_ES_INDEX
```
...
SkillRouter 不写 NETWORK_TRAFFIC_ES_INDEX
NETWORK_TRAFFIC_ES_INDEX 只给 RAG
