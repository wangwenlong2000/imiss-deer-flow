...
4. **Access the application**:
   - Web Interface: http://localhost:23976
   - API Gateway: http://localhost:23976/api/*
   - LangGraph: http://localhost:23976/api/langgraph/*
...
make docker-init
# Start Docker services (mode-aware, localhost:23976)
make docker-start
...
Docker Compose (deer-flow-dev)
  ├→ nginx (port 23976) ← Reverse proxy
  ├→ web (port 23975) ← Frontend with hot-reload
...
1. **Start backend services**:
   ...
   # Terminal 3: Start Frontend (port 23975)
   cd frontend
   pnpm dev
   ```
...
- Unified entry point on port 23976
- Routes `/api/langgraph/*` to LangGraph Server (2024)
- Routes other `/api/*` endpoints to Gateway API (8001)
- Routes non-API requests to Frontend (23975)
...
Browser
  ↓
Nginx (port 23976) ← Unified entry point
  ├→ Frontend (port 23975) ← / (non-API requests)
