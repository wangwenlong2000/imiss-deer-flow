...
The local compliance demo exposes five guide-aligned test profiles rather than
an arbitrary role/scene cross product: personal self-check (`self_use`), same
organisation processing (`internal_org`), authorised cross-organisation
collaboration (`cross_org`), public release (`public_release`), and anonymised
research (`research_anon`). Selecting a profile atomically binds its identity,
role, organisation and scene. The backend validates that binding again and
falls back to `_unknown` for a mismatched or tampered context. These profiles
exercise the 7.23 policy matrix; they remain test fixtures and do not replace a
