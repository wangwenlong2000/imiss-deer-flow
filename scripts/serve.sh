...
./scripts/wait-for-port.sh 23975 120 "Frontend" || {
    echo "  See logs/frontend.log for details"
    tail -20 logs/frontend.log
    cleanup
}
echo "✓ Frontend started on localhost:23975"

echo "Starting Nginx reverse proxy..."
nginx -g 'daemon off;' -c "$REPO_ROOT/docker/nginx/nginx.local.conf" -p "$REPO_ROOT" > logs/nginx.log 2>&1 &
NGINX_PID=$!
./scripts/wait-for-port.sh 23976 10 "Nginx" || {
    echo "  See logs/nginx.log for details"
    tail -10 logs/nginx.log
    cleanup
}
echo "✓ Nginx started on localhost:23976"
...
echo "=========================================="
echo ""
echo "  🌐 Application: http://localhost:23976"
echo "  📡 API Gateway: http://localhost:23976/api/*"
echo "  🤖 LangGraph:   http://localhost:23976/api/langgraph/*"
echo ""
echo "  📋 Logs:"
