...
# Allow multiple isolated checkouts owned by the same Unix user. If no explicit
# instance prefix is supplied, retain the original per-user behaviour.
CURRENT_USER="${DEER_FLOW_INSTANCE_PREFIX:-${SUDO_USER:-${USER:-$(id -un)}}}"
