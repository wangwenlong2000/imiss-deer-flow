...
2. copy 匹配文档的`source_path` 图片 到 `/mnt/user-data/outputs/` 目录下，命名为 `matched_image.jpg`，供用户进行人工验证和确认
``` bash
cp /mnt/nas/streetview_meta/database_gsv_vga/unzip/03823/382371.117_3946944.870/Pw7Mmj6np6aVMzeIOTGNIQ_012_330.png /mnt/user-data/outputs/matched_image.jpg
```
...
- **成功匹配场景：**
  ...
  - 使用 present_files 工具展示 `/mnt/user-data/outputs/` 目录下的 `证据.json` 文件和匹配图片 `matched_image.jpg`，供用户进行人工验证和确认
...
4. **结果验证：** **必须在答案中添加验证依据**，**必须使用 `present_files` 工具展示检索结果中的街景图片，供用户进行人工验证和确认**