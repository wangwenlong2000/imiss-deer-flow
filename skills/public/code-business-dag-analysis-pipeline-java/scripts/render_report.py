"""Render a standalone Markdown report from a Java code-business-DAG result."""
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "## 1. 摘要",
            "",
            f"本报告基于 `code-business-dag-analysis-pipeline-java` 对输入 Java 代码的静态分析结果生成。当前识别出的流水线类型为 `{pipeline_type}`。",
            "",
            ...
            "## 3. AST 与调用关系",
            "",
            "Java 版本优先调用 `code-to-ast-new` 的 Java 解析入口；如果当前解析器未实现或不可用，会降级为本技能内置的轻量静态扫描，继续提取方法调用序列和业务节点。",
            "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render a standalone Markdown report from a Java pipeline result JSON.")
