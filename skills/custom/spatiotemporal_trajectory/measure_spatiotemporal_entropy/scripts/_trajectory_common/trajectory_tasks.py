#!/usr/bin/env python3
"""
trajectory_tasks.py — trajectory 时空轨迹分析共享算法库
========================================================
...
设计原则：
  - 纯 Python 标准库，零第三方依赖
  - 输入支持 CSV / TSV / JSON / JSONL / trajectory evidence.jsonl 多种格式
...
def hash_uid(uid: str, salt: str = "trajectory_v1") -> str:
    return hashlib.sha256((salt + str(uid)).encode("utf-8")).hexdigest()[:12]


# ════════════════════════════════════════════════════════════════════
#  Geohash → 业务地名查表（与 spatiotemporal-search v2 兼容）
# ════════════════════════════════════════════════════════════════════
# 同 spatiotemporal-search/scripts/landmarks.py 一致：
...
def _load_landmarks() -> dict:
    ...
    # 本文件作为 spatial-temporal-skill-pack 的共享实现被根目录 skill 调用。
    # spatiotemporal-search 的真实数据目录是 skills/custom/spatiotemporal-search/skill/data/。
    candidates = [
        Path(__file__).resolve().parents[3] / "spatiotemporal-search" / "skill" / "data" / "geohash_landmarks.json",
        Path(__file__).resolve().parents[3] / "spatiotemporal-search" / "data" / "geohash_landmarks.json",
    ]
    for c in candidates:
        if c.exists():
            try:
...
# ════════════════════════════════════════════════════════════════════
#  字段自适应：在 trajectory evidence / 原始 GPS / region_heat 等
...
def _evidence_meta(rec: dict) -> dict:
    """trajectory evidence schema：信息嵌在 meta.{geo_scope, time_range, features}。"""
...
def flow_anomaly(
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    records = read_records(input)
    # group_col / metric_col 自适应：trajectory evidence 用 geohash + checkin_count
