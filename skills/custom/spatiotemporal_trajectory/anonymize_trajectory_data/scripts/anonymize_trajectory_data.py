...
def parse_timestamp(s: str) -> Optional[datetime]:
    ...
    for fmt in (
        ...
        "%Y-%m-%d %H:%M:%S",
        "%Y/%m/%d %H:%M:%S",
        "%a %b %d %H:%M:%S %z %Y",  # Twitter check-in 格式
    ):
        try:
    ...
    with open(evidence_path, "w", encoding="utf-8") as out_f:
        for (date_str, slot_name, ghash), b in sorted(buckets.items()):
            ...
            evidence = {
                ...
                "text": text,
                "meta": {
                    "source_id": f"trajectory_checkins_{city_label.lower()}",
                    "time_range": {
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="时空轨迹隐私脱敏：原始 GPS → 聚合 evidence",
    ...
    p.add_argument("--salt", default="trajectory_v1", help="哈希 salt")
