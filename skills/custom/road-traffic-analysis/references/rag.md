...
```bash
pip install requests sentence-transformers==5.4.1 PyMuPDF==1.27.2.3
```

## Data Scope

- Elasticsearch index: `xian_traffic_2024_min`
- Source chunks: `scripts/rag_db/xian2024_chunks.jsonl`
- Source document: `西安2024.pdf`
- Current coverage: 2024 Xi'an city traffic annual-report chunks

The script reads Elasticsearch connection settings from the environment:

- `ES_URL`
- `ES_USERNAME`
- `ES_PASSWORD`

If the Elasticsearch index has not been imported yet, run:
...
Retrieval mode defaults to `auto`: it uses vector retrieval when `sentence-transformers`
is installed, otherwise it imports and searches with Elasticsearch text search. To force
text search:
...
After installing `sentence-transformers`, rebuild the index with `--retrieval-mode vector`
if semantic vector retrieval is required.
...
For broad Xi'an historical/background questions about recent-years congestion or traffic
operation trends, call `rag_xian2024_min.py query` directly. Do not first ask for uploaded
CSV data or inspect `/mnt/user-data/workspace` unless the user explicitly asks for uploaded
data analysis.
