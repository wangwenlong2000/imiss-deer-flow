---
name: dataset-quality-and-linkability-diagnostic
description: 诊断电话网络数据集是否具备标准图结构、数据质量是否满足后续分析条件、各下游 skill 是否可用，并检查多省份数据的 ID 命名空间与跨省可联动性前提。适用于数据体检、能力矩阵、unified 是否支持跨省实体联动等问题；诊断结论不得被夸大为业务事实。
---
...
它不负责发现风险对象，也不负责生成业务研判结论，更不能把“数据不支持某类分析”解释成“现实中一定不存在该类关系”。它的作用是先判断一个已经处理好的电话网络图数据集是否具备后续分析条件，尤其用于回答：
...
- 如果跨省 ID 格式明显不同且没有交集，只能说明当前脱敏数据前提不支持可靠跨省同实体分析，不应推断为真实世界没有跨省联动。
