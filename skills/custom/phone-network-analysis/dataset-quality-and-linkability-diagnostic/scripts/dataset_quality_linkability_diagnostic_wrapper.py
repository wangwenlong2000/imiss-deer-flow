...
def build_capability_matrix(summary: Dict[str, Any], reg: Dict[str, Any], schema: Dict[str, Any], quality: List[Dict[str, Any]], linkability: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    matrix.append(row("dataset-quality-and-linkability-diagnostic", "supported" if cross_link_score >= 2 else "not_supported" if linkability.get("enabled") else "not_applicable", "存在跨省ID相等证据，可做有限跨省联动。" if cross_link_score >= 2 else linkability.get("conclusion_zh", "当前数据不适合跨省同实体联动分析。"), "若不支持，先补统一哈希规则/映射表，或仅做地域对比。"))
