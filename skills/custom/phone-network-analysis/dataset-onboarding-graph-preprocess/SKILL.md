---
name: dataset-onboarding-graph-preprocess
description: 将新上传的电话网络原始 CSV、Excel、JSON 或 Parquet 数据进行字段识别、清洗、质量检查和建图预处理，生成 user_nodes.csv、call_edges.csv、edges_phone_imei.parquet 等标准图结构文件。适用于原始数据接入、无法建图诊断和字段映射；已是标准图结构的数据应先走数据质量诊断或具体分析 skill。
---
