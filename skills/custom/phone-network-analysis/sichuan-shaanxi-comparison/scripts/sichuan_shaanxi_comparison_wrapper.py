...
def write_reports(
    ...
    if dfs.get("cross_device_examples") is not None and not dfs["cross_device_examples"].empty:
        report.append("\n### 跨省共享设备样例（用于后续 dataset-quality-and-linkability-diagnostic 下钻）\n")
    ...
    report.append("\n## 八、建议的后续分析\n")
    report.append("- 若要继续寻找跨省共享设备、跨省共同对端和跨省强关联对象，建议使用 `dataset-quality-and-linkability-diagnostic`。\n")
    ...
    pres.append("\n## 五、后续建议\n")
    pres.append("1. 使用 `dataset-quality-and-linkability-diagnostic` 继续核查跨省共享设备和共同对端。\n")
