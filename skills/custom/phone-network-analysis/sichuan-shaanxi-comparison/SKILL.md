---
name: sichuan-shaanxi-comparison
description: 比较四川与陕西电话网络数据在对象规模、风险标签、行为模式、共享设备规模、群体结构、时间覆盖和代表性对象上的地域差异。该 skill 只做地域对比和汇报材料，不做跨省同实体联动、跨省共享设备识别或跨省共同对端识别。
---
...
```text
请使用 sichuan-shaanxi-comparison skill，对四川和陕西进行地域对比，重点关注共享设备、设备池风险、群体结构差异和代表性风险对象。必须运行本 skill 的 wrapper 脚本，并输出后续可联动 shared-device-analysis、gang-cluster-analysis、dataset-quality-and-linkability-diagnostic 的建议。
```
...
5. 如果出现疑似跨省共享设备样例，不应直接判定为跨省同实体联动，应先交给 `dataset-quality-and-linkability-diagnostic` 做跨省可联动性诊断，并提醒当前 unified 数据不支持可靠跨省同实体联动。
...
1. `dataset-quality-and-linkability-diagnostic`：如用户要求跨省同实体联动，先做跨省可联动性诊断，并提醒当前 unified 数据不支持可靠跨省同实体联动。
