...
CAPABILITIES: dict[str, dict[str, Any]] = {
    ...
    "semantic_video_retrieval": {
        "label": "语义视频检索",
        "skills": ["video-search"],
        "deliverables": ["语义相似视频列表", "相似度或排序", "语义命中原因"],
        "keywords": ["语义", "streetmodel", "相似", "画面里", "傍晚", "下雨", "行人过马路", "交通拥堵", "变道", "可疑行为"],
    },
...
def select_capability(request: str) -> str:
    ...
    return winner if score > 0 else "video_asset_retrieval"
...
def follow_up_question(fields: list[str]) -> str | None:
    if not fields:
        return None
    if len(fields) == 1:
...
def build_steps(chain: list[str]) -> list[dict[str, str]]:
    labels = {
        "video-search": "Load video-search and query indexed videos with structured filters or semantic mode.",
        ...
        "video-embedding-index": "Load video-embedding-index to create or update vector search fields; run once and report any StreetModel failure without probing service health.",
...
def capability_gap(request: str, capability: str) -> dict[str, Any] | None:
    gaps = []
    if any(word in request for word in ["实时", "rtsp", "gb28181", "直播"]):
        gaps.append("Real-time stream connection is not covered by current local-video skills.")
    if any(word in request for word in ["同一人", "人脸识别", "身份识别"]):
        gaps.append("Cross-camera identity recognition requires a dedicated identity-matching capability.")
    if "热力图" in request:
        gaps.append("Heatmap visualization is only partially supported unless aggregated location bins already exist.")
    if any(word in request for word in ["ocr", "文字识别", "车牌识别"]):
...
def build_plan(request: str, role: str | None = None) -> dict[str, Any]:
    ...
    if capability == "video_library_governance":
        guardrails.append("For ordinary ingestion or ingest-and-embed, use metadata-only ingestion and preserve provided business metadata. Do not run object detection unless explicitly requested.")
    ...
    return {
        ...
        "follow_up_question": follow_up_question(missing),
