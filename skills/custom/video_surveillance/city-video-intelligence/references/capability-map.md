...
| `video_library_governance` | Make videos searchable and manageable | Ingest upload to Elasticsearch; batch import directory; generate StreetModel vectors; check storage; delete test data | files, directory, unified index, owner, ingestion mode | video ids, ingestion status, in-place embedding status, failures | `video-stream-ingestion`, `batch-video-ingestion`, `video-embedding-index` | Confirm destructive actions |
