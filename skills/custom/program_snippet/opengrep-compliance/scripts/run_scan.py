...
RULES_HEADER_RE = re.compile(r"(?m)^rules\s*:")


def is_rule_config_file(path: Path) -> bool:
    if path.suffix.lower() not in {".yaml", ".yml"}:
        return False
    if path.name.startswith("."):
        return False
    try:
        return RULES_HEADER_RE.search(path.read_text(encoding="utf-8", errors="ignore")) is not None
    except OSError:
        return False


def iter_rule_config_files(rules_dir: Path) -> list[Path]:
    return sorted(path for path in rules_dir.rglob("*") if path.is_file() and is_rule_config_file(path))


def prepare_runtime_rules_dir(source_rules_dir: Path, runtime_rules_dir: Path) -> tuple[int, int]:
    rule_files = iter_rule_config_files(source_rules_dir)
    yaml_files = [
        path
        for pattern in ("*.yaml", "*.yml")
        for path in source_rules_dir.rglob(pattern)
        if path.is_file()
    ]

    if runtime_rules_dir.exists():
        shutil.rmtree(runtime_rules_dir)
    runtime_rules_dir.mkdir(parents=True, exist_ok=True)

    for source in rule_files:
        relative_path = source.relative_to(source_rules_dir)
        target = runtime_rules_dir / relative_path
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, target)

    return len(rule_files), max(len(yaml_files) - len(rule_files), 0)
...
def render_markdown_report(summary: dict[str, Any], target: Path, rules_dir: Path) -> str:
    lines = [
        ...
        f"- Rules source directory: `{summary.get('source_rules_dir')}`",
        f"- Runtime rules directory: `{rules_dir}`",
        f"- Runtime rule files loaded: `{summary.get('runtime_rule_files')}`",
        f"- Ignored YAML/YML non-rule files: `{summary.get('ignored_yaml_files')}`",
...
def main() -> int:
    ...
    runtime_rules_dir = output_dir / ".opengrep-runtime-rules"
    ...
    runtime_rule_files, ignored_yaml_files = prepare_runtime_rules_dir(rules_dir, runtime_rules_dir)
    if runtime_rule_files == 0:
        raise FileNotFoundError(f"No OpenGrep rule config files found under: {rules_dir}")

    command = [
        str(binary),
        "scan",
        "--config",
        str(runtime_rules_dir),
        "--json-output",
        str(json_report),
        "--sarif-output",
        str(sarif_report),
    ...
    summary["source_rules_dir"] = str(rules_dir)
    summary["runtime_rules_dir"] = str(runtime_rules_dir)
    summary["runtime_rule_files"] = runtime_rule_files
    summary["ignored_yaml_files"] = ignored_yaml_files
    summary["stdout_tail"] = completed.stdout[-4000:]
    summary["stderr_tail"] = completed.stderr[-4000:]

    rendered_rule_report = render_markdown_report(summary, target, runtime_rules_dir)
