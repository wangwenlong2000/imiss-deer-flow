...
Before invoking OpenGrep, the runner filters `rules/` into an output-local runtime directory containing only `.yaml`/`.yml` files with a top-level `rules:` key. This prevents repository metadata such as `.pre-commit-config.yaml` from being loaded as rules.
...
```bash
python skills/custom/program_snippet/opengrep-compliance/scripts/run_scan.py --target . --output-dir reports/opengrep
```
