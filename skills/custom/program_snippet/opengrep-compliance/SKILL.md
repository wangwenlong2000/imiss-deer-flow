...
- Before invoking OpenGrep, filter the local rules directory to runtime rule configs only: `.yaml`/`.yml` files with a top-level `rules:` key. Do not pass repository metadata files such as `.pre-commit-config.yaml` directly to OpenGrep.
...
```bash
python /mnt/skills/custom/program_snippet/opengrep-compliance/scripts/run_scan.py \
  --target . \
...
The runner creates a filtered runtime rule directory under the output directory:

```text
reports/opengrep/.opengrep-runtime-rules/
```

This directory exists only to prevent non-rule YAML files from being loaded by OpenGrep.
