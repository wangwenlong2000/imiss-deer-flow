...
def _classify_previous_task_turn_with_llm(
    ...
    *,
    ...
    prompt = (
        ...
        "可选 act：\n"
        "- task_followup: 当前消息是在追问、解释、复盘、继续、修改、补充或评价上一轮任务/结果/执行过程。\n"
        "- new_task: 当前消息提出了新的业务对象、新场景、新分析目标，或需要独立启动一个新任务。\n"
        "- chitchat: 闲聊、感谢、无任务内容。\n"
        "- confirmation: 简短确认/否认，但没有 pending_action 可绑定。\n\n"
        "判断规则：\n"
        "1. 如果当前消息单独看像代码/脚本/政策/视频等任务，但结合上一轮是在问为什么这么做、要求改上一轮结果、继续上一轮分析，应判 task_followup。\n"
        "2. 只有用户明确引入新的分析对象、文件、场景或独立目标，才判 new_task。\n"
        "3. task_followup 时 should_reclassify_intent=false, should_reroute_skills=false。\n"
        "4. new_task 时 should_reclassify_intent=true, should_reroute_skills=true。\n\n"
        "输出 JSON schema:\n"
        "{\n"
