...
from deerflow.compliance.manual_context import validate_manual_context
...
class ManualSceneResolver:
    ...
    def _selected_scene(cls, raw: Any) -> Scene | None:
        profile, _ = validate_manual_context(raw)
        return profile.scene if profile is not None else None  # type: ignore[return-value]

    def resolve_context(self, raw: Any) -> SceneResolution:
        data = _mapping(raw)
        profile, validation_reason = validate_manual_context(data)
        if profile is None:
            return SceneResolution(
                source="manual_ui",
        ...
        return SceneResolution(
            ...
            confidence=0.5,
            source="manual_ui",
            reason_codes=("manual_profile_validated", "guide_7_23_scene_binding", "not_authoritative"),
            fallback=False,
            permission_checked=False,
            scene_hint=profile.scene,
        )
