...
All APIs are accessed through the Nginx reverse proxy at port 23976.
...
```
ws://localhost:23976/api/langgraph/threads/{thread_id}/runs/stream
```
...
client = get_client(url="http://localhost:23976/api/langgraph")
...
```bash
# List models
curl http://localhost:23976/api/models

# Get MCP config
curl http://localhost:23976/api/mcp/config

# Upload file
curl -X POST http://localhost:23976/api/threads/abc123/uploads \
  -F "files=@document.pdf"

# Enable skill
curl -X POST http://localhost:23976/api/skills/pdf-processing/enable

# Create thread and run agent
curl -X POST http://localhost:23976/api/langgraph/threads \
  -H "Content-Type: application/json" \
  -d '{}'

curl -X POST http://localhost:23976/api/langgraph/threads/abc123/runs \
  -H "Content-Type: application/json" \
  -d '{
    "input": {"messages": [{"role": "user", "content": "Hello"}]},
    "config": {"configurable": {"model_name": "gpt-4"}}
