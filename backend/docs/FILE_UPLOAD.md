...
```bash
# 1. 上传单个文件
curl -X POST http://localhost:23976/api/threads/test-thread/uploads \
...
# 3. 列出已上传文件
curl http://localhost:23976/api/threads/test-thread/uploads/list

# 4. 删除文件
curl -X DELETE http://localhost:23976/api/threads/test-thread/uploads/document.pdf
```
